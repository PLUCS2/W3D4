...
def vanity_projects
  ...
  Movie.select(:id, :title, "actors.name").joins(:actors).where("director_id = castings.actor_id AND ord = 1")
end
...
def most_supportive
  ...
  Actor.select(:id, :name, "COUNT(*) AS roles").joins(:castings).where("ord != 1").group("actors.id").order("roles DESC").limit(2)
