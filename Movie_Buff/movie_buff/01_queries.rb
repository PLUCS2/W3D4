...
def harrison_ford
  ...
  # appeared but not as a lead actor
   ...
   Movie.select(:id, :title).joins(:actors).where("actors.name = 'Harrison Ford' AND ord != 1")
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:castings).group('movies.id').order("COUNT(*) DESC").limit(3)
